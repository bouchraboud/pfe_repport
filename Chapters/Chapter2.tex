% !TEX root = ../main.tex
\chapter{Technical and Organizational Context}
\label{chap:chapter2}
\section*{Introduction}
As Oracle continues to expand the adoption of the Data Intelligence Services (DIS) platform across its Global Business Units (GBUs), the choice of tools, technologies, and infrastructure becomes increasingly important. This chapter provides the technical foundation necessary to understand the rationale behind the design and implementation of the DIS platform. It introduces the problem domain, presents a high-level overview of the system, discusses key architectural considerations, and highlights the core technologies that underpin DIS.

Because this internship focused on identifying and addressing integration challenges encountered by GBUs, the chapter first explains their role within Oracle. It then presents the position of DIS within the organization, its regional availability, and its purpose as a shared analytics platform. This background establishes the context required to understand the work carried out during the internship.

\section{Organizational Context within Oracle}
Oracle Corporation is structured around two major pillars:
\begin{itemize}
    \item the greater Oracle organization, which focuses primarily on infrastructure and platform services such as OCI.
    \item Global Business Units (GBUs), which deliver SaaS solutions tailored to specific industry domains such as healthcare, finance, hospitality, and retail.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/oracle entities.png}
    \caption{Oracle entities}
    \label{fig:oracle-entities}
\end{figure}

To support these GBUs with shared tools and infrastructure, Oracle established Shared Services. This organization includes Cloud Development Services (CDS), which develops common platforms and capabilities used across GBUs. The Data Intelligence Services team is part of CDS and helps bridge the infrastructure and application domains.

DIS provides a managed machine-learning and analytics platform for use across Oracle's GBUs. Instead of requiring each GBU to build and operate a separate data and AI platform, DIS offers centralized, reusable services. SaaS products can therefore integrate machine-learning and analytics capabilities without having to manage the underlying infrastructure independently. This shared approach improves consistency, reduces duplicated effort, and accelerates innovation across Oracle's industry-specific SaaS offerings.

\section{DIS Region Availability and Status}
DIS is intended to be deployed in the OCI regions where the required supporting services are available. Deployment priorities are determined according to forecast demand from GBU partners and their regional requirements. Figure~\ref{fig:dis-region-availability} presents the planned availability of DIS across the relevant regions.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/DIS Region Availability.png}
    \caption{DIS Region Availability}
    \label{fig:dis-region-availability}
\end{figure}

\section{What Is Data Intelligence?}
Data intelligence refers to an organization's ability to collect, process, analyze, and interpret data in order to generate actionable insights. It combines tools, technologies, governance practices, and analytical methods that transform raw data into information that supports decision-making. By using their data assets effectively, organizations can improve operational efficiency, identify opportunities, and support strategic initiatives. In a data-driven environment, these capabilities are important for sustaining innovation and delivering value across business units.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{Logos/data intelegence logo.png}
    \caption{Data intelligence logo}
    \label{fig:data-intelligence-logo}
\end{figure}

\section{DIS Purpose and Role}
DIS is an internal Oracle platform that provides product teams with a reusable analytics stack. Instead of requiring each GBU to build its analytics infrastructure from the ground up, DIS provides shared capabilities for data ingestion, storage, processing, machine learning, and visualization. A partner SaaS application can therefore integrate dashboards, reports, predictive models, and analytics pages through DIS rather than developing and operating each capability independently.

From a business perspective, Oracle's GBUs deliver SaaS solutions to their customers, while DIS provides the analytics layer embedded within those offerings. The platform provisions and manages services such as Oracle Analytics Server (OAS), Autonomous Data Warehouse (ADW), and Apache Spark, together with other analytics and data-processing components. End users access these capabilities through the corresponding SaaS applications.

By leveraging DIS for analytics-intensive workloads, partners benefit from the following advantages:

\begin{itemize}
    \item Avoid building and maintaining a custom analytics technology stack.
    \item Gain access to a cost-effective and scalable architecture tailored to analytics requirements.
    \item Focus on developing advanced machine learning capabilities using built-in frameworks and libraries.
    \item Maintain full control and flexibility over data ingestion, storage, processing, and visualization.
    \item Reduce Cloud Service Security Approval Process (CSSAP) overhead by adopting DIS's pre-approved architectural patterns.
\end{itemize}

\subsection{Multi-Tenancy and Resource Isolation}

Multi-tenancy allows DIS to serve several partners and customer
environments while sharing common platform services. In this context, a
\textbf{tenant} represents an isolated logical environment with its own
identifier, configuration, enabled services, identities, analytics
resources, connections, and operational status. A \textbf{partner}
represents the GBU or SaaS application that uses DIS and may manage several
tenants.

An OCI tenancy is different from a DIS tenant. The OCI tenancy is the
cloud-account and root-resource boundary containing infrastructure such as
compartments, networks, Kubernetes clusters, databases, and Object Storage.
A DIS tenant is created at the application-platform level and represents
one logically isolated customer or application environment. Figure~\ref{fig:oci-tenancy-dis-tenant}
illustrates these different levels.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]
    {Figures/oci_tenancy_vs_dis_tenant.png}
    \caption{Difference between an OCI tenancy and a DIS tenant}
    \label{fig:oci-tenancy-dis-tenant}
\end{figure}

During onboarding, DIS provisions only the components declared in each
tenant's deployment template, such as a data lake, OAS, or OAC. Isolation
is enforced through tenant-specific identities and permissions, controlled
resource organization, network rules, and separate data and configuration
contexts. Consequently, operations performed for one tenant should not
affect the resources or data of another tenant.

\subsection{Partner and Tenant Provisioning Lifecycle}

Before a tenant can consume analytics services, DIS follows a controlled
provisioning lifecycle. This lifecycle separates the configuration shared
by a partner from the resources dedicated to each tenant:

\begin{enumerate}
    \item \textbf{Partner registration:} the partner is declared in DIS
    together with its authorized products, deployment templates, service
    limits, and infrastructure configuration.

    \item \textbf{Partner onboarding:} versioned application artifacts and
    container images are validated and stored so that they can later be
    deployed consistently for the partner's tenants.

    \item \textbf{Tenant registration:} a tenant identifier is created and
    associated with the corresponding partner. This identifier isolates the
    tenant's configuration and tracks subsequent lifecycle operations.

    \item \textbf{Tenant onboarding:} the partner submits an onboarding
    request that specifies the required products and deployment templates.
    The Orchestration Service validates the request and coordinates the
    services that provision and configure the requested resources.
\end{enumerate}

\subsubsection{Partner Artifact Packaging}

Before tenant provisioning, the partner prepares product-specific
configuration and analytics resources, such as semantic models,
visualization content, metadata, and deployment configuration. These
resources are packaged into a versioned container image based on a
platform-approved base image.

A continuous integration pipeline builds and publishes the image to an
authorized container registry. The DIS partner-artifact onboarding pipeline
then retrieves the image, validates its structure and metadata, and stores
its contents in the platform's artifact repository. The validated artifacts
can subsequently be used during tenant onboarding and upgrade operations.
This mechanism provides consistent packaging, traceable versioning, and
repeatable deployment across tenant environments.

Figure~\ref{fig:tenant-onboarding-workflow} summarizes the tenant onboarding
workflow, from submission of the request to the creation and configuration
of the tenant environment.

\begin{figure}[H]
    \centering
    \IfFileExists{Figures/tenant_onboarding_workflow.png}{%
        \includegraphics[width=0.6\textwidth]
        {Figures/tenant_onboarding_workflow.png}%
    }{%
        \fbox{\parbox[c][5cm][c]{0.88\textwidth}{\centering
        Insert the sanitized Tenant Onboarding Workflow figure here.}}%
    }
    \caption{Tenant Onboarding Workflow}
    \label{fig:tenant-onboarding-workflow}
\end{figure}

Listing~\ref{lst:tenant-onboarding-request} presents a simplified tenant
onboarding request. Sensitive values are replaced with generic placeholders,
while the main structure is preserved. The deployment template determines
which platform components must be provisioned for the tenant.

\begin{lstlisting}[style=mystyle,
caption={Simplified tenant onboarding request},
label={lst:tenant-onboarding-request}]
{
  "identityProviderUrl": "https://identity.example.com",
  "applicationClientId": "CLIENT_ID_PLACEHOLDER",
  "applicationSecret": "SECRET_PLACEHOLDER",
  "products": [
    {
      "name": "analytics-product",
      "version": "1.0.0"
    }
  ],
  "deploymentTemplate": [
    "dataLake",
    "oac"
  ],
  "dataLake": [
    "managedDatabase",
    "objectStorage"
  ],
  "oac": [
    {
      "instanceId": "analytics-instance",
      "capacity": {
        "ocpus": 2
      }
    }
  ]
}
\end{lstlisting}

After the workflow completes, DIS returns the provisioning status and the
non-sensitive information needed to access the enabled services. A simplified
response is shown in Listing~\ref{lst:tenant-onboarding-response}.

\begin{lstlisting}[style=mystyle,
caption={Simplified successful onboarding response},
label={lst:tenant-onboarding-response}]
{
  "status": "SUCCESS",
  "message": "Tenant onboarding completed",
  "lastUpdatedTimestamp": "YYYY-MM-DD HH:MM:SS",
  "platformVersion": "PLATFORM_VERSION",
  "products": [
    {
      "name": "analytics-product",
      "version": "1.0.0"
    }
  ],
  "oac": [
    {
      "instanceId": "analytics-instance",
      "analyticsUrl": "https://analytics.example.com",
      "status": "ACTIVE"
    }
  ],
  "dataLake": {
    "database": {
      "status": "AVAILABLE"
    },
    "objectStorage": {
      "status": "AVAILABLE"
    }
  }
}
\end{lstlisting}

If a provisioning step fails, the platform records the failed step and the
resulting resource state. According to that state, the workflow can resume
from a safe step, roll back partially created resources before resuming, or
require manual review. This recovery behavior is examined in
Chapter~\ref{chap:chapter3} in the context of cross-region disaster recovery.

\section{DIS Architecture Overview}

Oracle Global Business Unit Analytics (GBUA), also known as the Data Intelligence Service (DIS), is a managed service platform focused on delivering scalable, cloud-native data analytics and machine learning capabilities for Oracle SaaS applications. It enables business units to integrate and leverage machine learning (ML)-based insights without managing the underlying infrastructure or analytics tooling.

The DIS platform architecture can be logically divided into three core layers, as shown in Figure~\ref{fig:gbu_architecture}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/gbu_architecture.png}
    \caption{GBU Analytics and Machine Learning Architecture}
    \label{fig:gbu_architecture}
\end{figure}

\subsection{Data Ingestion}

The data ingestion layer supports multiple methods for collecting data from Oracle SaaS applications and partner systems:

\begin{itemize}
    \item \textbf{REST Ingestion:} Data can be streamed into the platform using REST endpoints. These endpoints perform inline validation and Personally Identifiable Information (PII) detection during ingestion.

    \item \textbf{Oracle Data Integrator (ODI):} ODI is used to extract and load data from partner Database-as-a-Service (DBaaS) systems. It supports standard Extract, Transform, and Load (ETL) operations.

    \item \textbf{Direct Upload to Object Storage:} Large datasets, such as log files and historical data, can be uploaded directly to Oracle Object Storage for later processing.
\end{itemize}

\subsection{Data Processing}

The data processing layer provides distributed data processing using Apache Spark. It executes machine learning pipelines and data transformation jobs, storing the processed results in Oracle DBaaS. Spark jobs are tenant-isolated and are designed to periodically retrain machine learning models using historical data stored in Object Storage.

\subsection{Visualization and Data Analysis}

After data has been processed and stored, Oracle Business Intelligence Enterprise Edition (OBIEE) or Oracle Analytics Cloud (OAC) is used for report generation and dashboard visualization. Access is managed through tenant Identity Cloud Service (IDCS), and reports are customized according to user roles. OBIEE also provides self-service analytics capabilities, enabling administrative users with limited SQL or data modeling expertise to create reports and perform data analysis.

\section{Cloud-Native Architecture of DIS}

Cloud-native architecture refers to designing applications specifically
for cloud environments using independently deployable services,
containers, automated orchestration, and API-based communication. These
principles improve scalability, resilience, deployment flexibility, and
operational automation.

DIS applies these principles through the following elements:

\begin{itemize}
    \item \textbf{Microservices:} platform responsibilities are divided
    among focused services that can evolve and scale independently.

    \item \textbf{Containers:} services and their dependencies are
    packaged consistently for deployment across environments.

    \item \textbf{Kubernetes orchestration:} Oracle Kubernetes Engine
    manages service deployment, scaling, availability, and recovery.

    \item \textbf{API-based communication:} REST APIs provide controlled
    communication between platform services.

    \item \textbf{CI/CD automation:} automated pipelines build, test,
    and deploy individual services with limited impact on the rest of the
    platform.
\end{itemize}

Figure~\ref{fig:cloud-native-architecture} summarizes the principal
building blocks of a cloud-native platform and their relationship with
automated software delivery and operations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]
    {Figures/cloud-native-architecture.jpg}
    \caption{Principal components of cloud-native architecture}
    \label{fig:cloud-native-architecture}
\end{figure}

DIS combines OCI infrastructure and managed services, including
networking, Object Storage, databases, and identity services, with custom
microservices deployed on Kubernetes. This architecture allows the
platform to support different analytics workloads while maintaining
scalability, tenant isolation, and operational reliability.

\section{DIS Microservices Overview}
The DIS platform implements a comprehensive microservices-based architecture in which loosely coupled services communicate through RESTful APIs. All services are containerized and deployed on the Oracle Kubernetes Engine (OKE) platform, enabling scalability, fault tolerance, and independent service deployment.

Figure~\ref{fig:dis_microservices} illustrates the high-level microservices architecture of the DIS platform.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/dis_microservices.png}
    \caption{DIS Microservices Architecture}
    \label{fig:dis_microservices}
\end{figure}

\subsection{Orchestration Service}

The \textbf{Orchestration Service} is responsible for automating the entire lifecycle of platform components and tenant environments, including initial deployment, configuration, ongoing management, updates, and rollback operations. It coordinates these tasks using template-driven workflows and internal REST APIs, ensuring that all provisioning activities are executed consistently and reliably.

This microservice serves as the central coordinator of the DIS platform by orchestrating interactions among the different services involved in tenant onboarding and lifecycle management. By automating complex operational workflows, it minimizes manual intervention, improves deployment consistency, and enhances the overall reliability of the platform.

The features studied in this project involve the Orchestration Service together
with the configuration and OAC execution services. Their interaction is
examined in Chapter~\ref{chap:chapter3} through the cross-region disaster
recovery workflow.

\subsection{Authentication Service}

The \textbf{Authentication Service} is responsible for managing the secure onboarding and provisioning of tenant identities within the DIS platform. It enforces authentication, authorization, and compliance policies by integrating with Oracle Identity Cloud Service (IDCS) and partner platforms through standardized REST APIs.

This service plays a critical role in ensuring that only authenticated and authorized tenants can access DIS resources. Throughout the provisioning and tenant lifecycle management processes, the Orchestration Service interacts with the Authentication Service to validate partner credentials, provision tenant identities, and manage access permissions. This collaboration guarantees secure communication between services while maintaining compliance with the platform's security requirements.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figures/Auth Service Sequence Diagram.png}
    \caption{Auth Service Sequence Diagram}
    \label{fig:Auth Service Sequence Diagram}
\end{figure}

\section{Development Environment and Tools}

The development workflow combines local source-code tools, API validation
utilities, continuous-integration services, container technologies, and
Kubernetes operational tools. Together, they support the complete engineering
cycle from source-code analysis to deployment verification in a non-production
environment. Each tool category is summarized by a compact illustration to
avoid repeating a separate logo after every description.

\newcommand{\toolfigure}[4]{%
\begin{figure}[H]
    \centering
    \IfFileExists{Figures/#1}{%
        \includegraphics[width=#2\textwidth]{Figures/#1}%
    }{%
        \fbox{\parbox[c][2.8cm][c]{0.55\textwidth}{\centering
        Reserved space for the #3 illustration.}}%
    }
    \caption{#3}
    \label{#4}
\end{figure}%
}

\subsection{Code Editors and Integrated Development Environments}

\begin{enumerate}
\renewcommand{\labelenumi}{\alph{enumi}}
\item \textbf{PyCharm}\par
PyCharm is an integrated development environment specialized in Python. It
provides code completion, navigation, debugging, refactoring, dependency
management, and direct test execution. These capabilities help developers
understand an existing microservice and modify its Python modules while
detecting syntax and type-related problems early.

\item \textbf{Visual Studio Code}\par
Visual Studio Code is a lightweight, cross-platform source-code editor that can
be extended for different languages and file formats. It was useful for
working with Python source code, JSON payloads, YAML deployment templates,
shell scripts, and LaTeX report files from the same interface. Its integrated
terminal and source-control view also simplify navigation between code changes
and command-line verification.
\end{enumerate}

\toolfigure{tools_code_editors.png}{0.4}{Code editors used during development}{fig:tools-code-editors}

\subsection{Programming, Web Services, and Testing}

\begin{enumerate}
\renewcommand{\labelenumi}{\alph{enumi}}
\item \textbf{Python}\par
Python is a high-level programming language valued for its readable syntax and
large software ecosystem. It is the principal implementation language of the
OAC onboarding service and several related DIS microservices. In this work, it
supports REST request processing, workflow decisions, cloud-service
integration, status management, and automated tests.

\item \textbf{Flask}\par
Flask is a lightweight Python web framework used to create HTTP services
without imposing a large application structure. It initializes the service,
maps REST endpoints to Python handlers, processes incoming requests, and
returns structured responses. Its modular design is suitable for
microservices, where each service exposes a focused set of responsibilities.

\item \textbf{pytest}\par
pytest is a Python testing framework used to verify individual functions and
service behaviours automatically. It supports reusable fixtures,
parameterized scenarios, exception assertions, and mocking of external
dependencies. This makes it possible to test success, failure, retry, and
rollback decisions without provisioning real cloud resources for every test.
\end{enumerate}

\toolfigure{tools_programming_testing.png}{0.5}{Programming and testing technologies}{fig:tools-programming-testing}

\subsection{Version Control, Continuous Integration, and Containers}

\begin{enumerate}
\renewcommand{\labelenumi}{\alph{enumi}}
\item \textbf{Git}\par
Git is a distributed version-control system that records the history of source
files and allows changes to be developed independently. Feature branches were
used to isolate each story, while commits documented incremental
implementation steps. Git also enables comparison, review, and controlled
integration of changes into a shared codebase.

\item \textbf{GitLab}\par
GitLab is the collaborative platform used to host Git repositories and manage
the software review process. Merge requests present the differences between a
feature branch and its target branch, allowing reviewers to add comments and
request corrections. It also preserves links between source changes,
pipelines, approvals, and implementation history.

\item \textbf{Jenkins}\par
Jenkins is an automation server used to implement continuous-integration and
delivery pipelines. A pipeline installs dependencies, performs syntax and
quality checks, executes automated tests, builds the application package, and
creates a deployable container image. Successful automation provides a
repeatable path from reviewed source code to an image that can be validated in
a Kubernetes environment.

\item \textbf{Docker}\par
Docker provides the image format and build instructions used to package an
application with its runtime, dependencies, configuration paths, and required
resources. The resulting image offers a consistent execution environment
across developer, integration, and deployment systems. This reduces problems
caused by differences between the machines on which the service runs.

\item \textbf{Podman}\par
Podman is a daemonless container engine compatible with common container-image
workflows. It can build images from a Dockerfile, run containers, assign tags,
and publish images to a registry. The project pipelines use these operations
to build service images in an automated and security-conscious environment.

\item \textbf{Oracle Cloud Infrastructure Registry}\par
Oracle Cloud Infrastructure Registry is a managed private registry for storing
and distributing container images. Each pipeline publication is identified by
a repository path and version tag, allowing Kubernetes deployments to select
a precise service build. Registry access controls also prevent unauthorized
users or environments from pulling or publishing internal images.
\end{enumerate}

\toolfigure{tools_cicd_containers.png}{0.3}{Version control, continuous integration, and container tools}{fig:tools-cicd-containers}

\subsection{API Development and Validation}

\begin{enumerate}
\renewcommand{\labelenumi}{\alph{enumi}}
\item \textbf{Bruno}\par
Bruno is an API client used to organize and execute HTTP requests. Request
collections can contain endpoint paths, headers, JSON payloads, authentication
information, and environment variables. During validation, it was used to
submit onboarding and operational requests and inspect their status codes and
response bodies in a reproducible way.

\item \textbf{cURL}\par
cURL is a command-line data-transfer utility commonly used to call REST APIs.
It can specify the HTTP method, URL, request headers, authentication token, and
JSON body in a single command. It is useful for quick endpoint verification,
debugging from a bastion host, and integration into shell-based automation.

\item \textbf{Swagger/OpenAPI}\par
OpenAPI defines a machine-readable contract describing a REST service, while
Swagger tools render that contract as interactive documentation. The interface
shows available endpoints, parameters, request models, response models, and
status codes. It helps developers confirm the expected API contract and test
requests without reading the complete service implementation.
\end{enumerate}


\toolfigure{tools_api_development.png}{0.4}{API development, documentation, and validation tools}{fig:tools_api_development}

\subsection{Deployment and Operational Validation}

\begin{enumerate}
\renewcommand{\labelenumi}{\alph{enumi}}
\item \textbf{Kubernetes}\par
Kubernetes is a container-orchestration platform that automates deployment,
scheduling, health monitoring, and replacement of containerized services. DIS
uses Kubernetes resources such as Deployments, Pods, Services, ConfigMaps,
Secrets, persistent volumes, and CronJobs. It therefore provides the runtime
environment in which a new service image is deployed and operationally
validated.

\item \textbf{Linux bastion host}\par
A Linux bastion host is a controlled administrative entry point placed between
an engineer and private cloud resources. After authentication, it provides a
terminal from which authorized Kubernetes commands and diagnostic operations
can be executed. This approach limits direct exposure of the cluster and
centralizes operational access through a protected environment.

\item \textbf{OCI Object Storage}\par
Oracle Cloud Infrastructure Object Storage is a durable service for storing
files as objects inside logical buckets. Within DIS workflows, it stores
tenant artifacts, configuration objects, execution-status information, and
OAC snapshot or BAR backups. Its object-based model and cross-region
capabilities make it important for both routine platform operations and
Disaster Recovery preparation.
\end{enumerate}

\toolfigure{tools_deployment_operations.png}{0.3}{Deployment and operational tools}{fig:tools-deployment-operations}

\section*{Conclusion}

This chapter positioned DIS within Oracle's organizational and technical ecosystem. It explained how the platform supports GBUs by providing shared, scalable analytics and machine-learning services across OCI regions. By centralizing these capabilities, DIS reduces duplicated infrastructure, promotes consistent architectural practices, and enables product teams to focus on customer-facing functionality.
